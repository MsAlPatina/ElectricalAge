...
public class AdvancedElectricalMathElement extends SixNodeElement implements IConfigurable {
    ...
    public AdvancedElectricalMathElement(SixNode sixNode, Direction side, SixNodeDescriptor descriptor) {
        ...
        powerLoad.setSerialResistance(MnaConst.noImpedance);
    ...
    private void updatePowerUse(){
        ...
        if (isPowered) {
            powerResistor.setResistance((lastU * lastU) / powerNeeded);
        } else {
            powerResistor.setResistance((lastU * lastU) / wattsStandBy);
        }
    }
            ...
            for (int i = 0; i <= 0xF; i++) {
                ...
                if ((mask & (1 << i)) != 0){
                    st.append(ElectricalSignalBusCableElement.Companion.getWool_to_chat()[15 - i].toString())
                        .append(Utils.plotVolt(gateInput[idx][i].getVoltage()));
                }
            }
    ...
    public Map<String, String> getWaila() {
        ...
        info.put(I18N.tr("Input voltage"), Utils.plotVolt("", gateOutput[0].getVoltage()));
        info.put(I18N.tr("Input power"), Utils.plotPower("", powerResistor.getPower()));
        return info;
    }
    ...
    public String multiMeterString() {
        ...
        builder
            .append("Input: ")
            .append(Utils.plotVolt("U:", powerLoad.getVoltage()))
            .append(Utils.plotPower("P:", powerResistor.getPower()));
